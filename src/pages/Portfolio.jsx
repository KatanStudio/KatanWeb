    ...
    <section className="section section--orbs" id="portfolio">
      ...
      <div className="container">
        ...
        <div className="portfolio-carousel">
          <button className="portfolio-carousel__arrow" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} aria-label="Página anterior">&#8592;</button>
          <div className="portfolio-grid">
            {currentItems.map((project) => (
              <a key={project.title} href={project.href} className="portfolio-card" data-category={project.category} target={project.external ? '_blank' : undefined} rel={project.external ? 'noreferrer' : undefined}>
                <div className="portfolio-card__img-wrap">
                  <img src={project.img} alt={project.alt} className="portfolio-card__img" />
                  <div className="portfolio-card__overlay"><span className="btn btn--primary btn--chamfer">Ver proyecto</span></div>
                </div>
                <div className="portfolio-card__info">
                  ...
                  {project.technologies ? (
                    <div className="portfolio-card__techs">{project.technologies.map(tech => (<span key={tech} className="portfolio-card__tech-chip">{tech}</span>))}</div>
                  ) : (
          ...
          <button className="portfolio-carousel__arrow" onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} aria-label="Página siguiente">&#8594;</button>
        </div>
        {totalPages > 1 && (
          <div className="portfolio-carousel__dots">
            {Array.from({ length: totalPages }).map((_, i) => (
              <button key={i} className={`portfolio-carousel__dot${i === page ? ' portfolio-carousel__dot--active' : ''}`} onClick={() => setPage(i)} aria-label={`Ir a página ${i + 1}`}>{i + 1}</button>
            ))}
          </div>
        )}

        <div className="portfolio-swipe-wrapper">
          <button className="portfolio-swipe__arrow portfolio-swipe__arrow--prev" onClick={() => scrollToCard(Math.max(0, mobileIndex - 1))} disabled={mobileIndex === 0} aria-label="Proyecto anterior">&#8592;</button>
          <div className="portfolio-swipe" ref={swipeRef}>
            {visible.map((project) => (
              <div key={project.title} className="portfolio-swipe__item">
                <a href={project.href} className="portfolio-card" data-category={project.category} target={project.external ? '_blank' : undefined} rel={project.external ? 'noreferrer' : undefined}>
                  <div className="portfolio-card__img-wrap">
                    <img src={project.img} alt={project.alt} className="portfolio-card__img" />
                    <div className="portfolio-card__overlay"><span className="btn btn--primary btn--chamfer">Ver proyecto</span></div>
                  </div>
          ...
          <button className="portfolio-swipe__arrow portfolio-swipe__arrow--next" onClick={() => scrollToCard(Math.min(visible.length - 1, mobileIndex + 1))} disabled={mobileIndex >= visible.length - 1} aria-label="Proyecto siguiente">&#8594;</button>
        </div>
        <div className="portfolio-swipe__dots">
          {visible.map((_, i) => (
            <button key={i} className={`portfolio-carousel__dot${i === mobileIndex ? ' portfolio-carousel__dot--active' : ''}`} onClick={() => scrollToCard(i)} aria-label={`Ir al proyecto ${i + 1}`}>{i + 1}</button>
          ))}
        </div>
      </div>
    </section>
...
export default function Portfolio() {
  ...
  return (
    <>
      ...
      <main>
        <section className="section section--orbs" id="portfolio" style={{ paddingTop: '7rem' }}>
          <div className="glow-orb orb-page-1" aria-hidden="true" />
          <div className="glow-orb orb-page-2" aria-hidden="true" />
          <div className="container">
            <header className="section__header" style={{ textAlign: 'center' }}>
